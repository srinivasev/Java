...
public class CustomDeadlockSolution {
	...
	public static Thread thread2 = new Thread("Thread 2"){
		...
		public void run() {
			// TODO Auto-generated method stub
			synchronized (Lock1) {
				System.out.println("Obtaining lock 1 "+thread2.getName());
				try{
					Thread.sleep(10);
				}catch(InterruptedException e){
					e.printStackTrace();
				}
				System.out.println("Trying to get lock 2 "+thread2.getName());
				synchronized (Lock1) {
					System.out.println("Obtaining lock 1 lock 2 "+thread2.getName());
				}
			}
		}
	};
